...
def xaviar_initalization():
	...
	for i in range(2):
		w.append([])
		for j in range(2):
			w[i].append(r.normal(loc=0, scale=1/0.5))
		w[i].append(0) # for the bias
...
def backpropagation_dataset(v, w, dataset, y_axis):
	...
	y = [i[2] for i in dataset] # get all the y-values from the dataset as a vector
...
def main():
	...
	#x_axis, y_axis = rms_prop(w, v, dataset, 0.2, 0.5, 20)
	#del x_axis[0:2]
	#del y_axis[0:2]
	x_axis2, y_axis2 = rms_prop(w2, v2, dataset, 0.2, 0.5, 100)
